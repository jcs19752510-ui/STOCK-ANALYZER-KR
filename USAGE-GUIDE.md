...
2. 11단계 실제 배포 직전 승인 (규칙 E — 시스템상 강제, 우회 불가)
3. (선택) "이 산출물은 꼭 검토하고 싶다"고 사용자가 미리 지정한 지점 — 예: "2단계 기획서 나오면 나한테 보여주고 잠깐 멈춰줘" 라고 처음 요청에 넣으면 그 지점에서만 멈춘다

이 세 가지가 아니면, 5~7단계(개발→단위테스트→통합테스트)가 작업 단위 수만큼 반복되는 것도, 8~10단계(전체테스트→보안→배포테스트)로 이어지는 것도 전부 자동으로 진행된다. **11단계 배포만은 예외 없이 명확한 승인을 물어본다** ("지금 프로덕션에 배포해도 됩니다" 같은 구체적 문구가 있어야 함). 승인하면 배포 후 자동으로 12단계(배포후 검증)까지 이어진다.
...
정말 판단이 필요한 질문이나 11단계 배포 승인 시점이 아니면 멈추지 말고 계속 이어서 진행해줘.
```
