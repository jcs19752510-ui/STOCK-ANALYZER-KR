...
# 병렬 처리 (선택, ORCHESTRATOR.md 5-4 참고)
이 4개 문서는 서로 완전히 독립적이다 — 각각 08·09·10 산출물만 참조하고 서로를 참조하지 않는다. 오케스트레이터가 이 에이전트를 문서별로 범위를 지정해(예: "이번 호출은 `11-user-manual.md`만 작성") 최대 4회 병렬 호출하면, 이 에이전트는 지정된 문서 하나만 작성하고 나머지 3개는 손대지 않는다. 범위 지정 없이 호출되면 기존대로 4개 전부를 순서대로 작성한다.
