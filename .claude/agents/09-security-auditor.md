...
위 점검 항목(인증/인가, 인젝션, 시크릿/자격증명, 의존성CVE·라이선스, 개인정보 컴플라이언스, 규제·AI 규칙 반영)을 카테고리별로 서로 다른 `Agent` 호출로 나눠 동시에 점검한 뒤 하나의 `09-security-audit.md`로 합칠 수 있다. 09는 08 PASS 이후에만 시작한다는 순서(감사 추적성 확보)는 병렬화 대상이 아니다.
