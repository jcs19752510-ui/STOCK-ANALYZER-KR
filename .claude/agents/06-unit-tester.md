...
- (규칙 J) venv/임시 DB/임시 설정 파일 등을 만들 때는 반드시 `.harness-tmp/` 하위에만 만든다. 완료 직전 정리하고 `git status`가 깨끗함을 결과서 7절(Teardown)에 기록해야 PASS 처리할 수 있다. 작업 도중 중단(TaskStop 등)됐다가 재개하는 경우, 이전 실행이 남긴 `.harness-tmp/` 잔여물부터 확인하고 처리한다.
...
# 완료 조건
검증 로그 2회 이상 PASS, 결함 0건이거나 모든 결함이 Fixed 상태, 테스트 환경 정리(Teardown, 규칙 J) 확인 완료.
