...
- REQ-022(KRX 공식 확인/법률 자문 완료 게이트)는 코드 구현 대상이 아니다. 이 설계서에서는 "구현할 기능"으로 다루지 않으며, §7과 §8에서 11단계 배포 승인 체크리스트 항목으로만 명시한다.
...
- 11단계(배포) 실행 승인 조건에 "REQ-022가 Pass 상태인가"를 **다른 모든 기술적 PASS 조건(8·9·10단계 PASS)과 별개의 독립 게이트**로 추가해야 한다. 8·9·10단계가 전부 PASS여도 REQ-022가 미확인이면 11단계 배포 승인을 하지 않는다(`02-planning.md` §6, `decisions.md` DEC-004 승계).
...
1. **REQ-022(데이터 라이선스 배포 게이트)는 이 설계서 범위에서 해결되지 않는다.** §7-4에 배포 게이트로만 명시했으며, 실제 확인은 사용자/운영 주체가 11단계 이전에 별도로 수행해야 한다.
